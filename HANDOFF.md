...
**Problem (verified):** in `sieve-ingest/sieve_ingest/registry.py`, these sources will crawl **nothing** because they have no `sitemap_url`: `perplexity-docs, openai-docs, bing-webmaster, w3c, semrush-blog, search-engine-land`. And `google-search-central` uses the `changelog` adapter that hashes **one page** (`developers.google.com/search`), not the SEO/AEO doc corpus.
**Fix:**
1. Add a `url_list` **adapter** to `freshness.py` (`_ADAPTERS`): given `source.seed_urls` (a JSON array of doc URLs), for each URL do the same conditional-GET + content-hash as `_detect_sitemap`. This reuses everything; it's ~30 lines.
2. Add a `seed_urls jsonb` column to `source_registry` (in `db.py` CONTROL_SCHEMA).
3. In `registry.py`, set `adapter_type='url_list'` + a curated `seed_urls` for:
   - **Google SEO/AEO**: structured-data intro + each schema type page, crawling/indexing basics, JS SEO, sitemaps, canonicalization, **AI features / AI Overviews doc**, E-E-A-T / helpful-content, robots meta. (~15–25 canonical `developers.google.com/search/docs/...` URLs.)
   - **Perplexity**: enumerate `docs.perplexity.ai` pages (small site).
   - **OpenAI**: GPTBot / SearchGPT crawler + search docs.
   - **Bing/IndexNow, W3C, Semrush, SEL**: their real sitemaps or curated URL lists.
**Acceptance:** `python -m sieve_ingest run` writes new Google/Perplexity rules with `source_url` pointing at the exact doc page; auditor `query_brain('F1_...')` cites them.
